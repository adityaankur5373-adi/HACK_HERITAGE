...
export async function selectReportUniversities(req, res) {
  ...
  try {
    ...
    await prisma.$transaction([
      prisma.universityRecommendation.updateMany({ where: { reportId: assignment.reportId, status: "RECOMMENDED", universityId: { notIn: selectedIds } }, data: { status: "NOT_SELECTED" } }),
      prisma.universityRecommendation.updateMany({ where: { reportId: assignment.reportId, universityId: { in: selectedIds } }, data: { status: "SELECTED" } }),
      prisma.universityProblemInvitation.createMany({
      ...
      prisma.universityNotification.updateMany({
        where: {
          reportId: assignment.reportId,
      ...
      prisma.universityNotification.createMany({
        data: selectedIds.map((universityId) => ({
          universityId,
          reportId: assignment.reportId,
          type: "COLLABORATION_INVITATION",
    ...
    const recommendationsAfterSelection = await prisma.universityRecommendation.findMany({
