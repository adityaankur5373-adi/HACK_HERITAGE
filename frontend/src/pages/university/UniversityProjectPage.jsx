    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-8">
        ...
        <section className="mt-6 rounded-2xl border bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-4">{project.solutions?.map((solution) => <article key={solution.id} className="rounded-xl border p-4"><div className="flex flex-wrap items-center justify-between gap-3"><h3 className="font-bold">{solution.title}</h3><span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-800">{statusLabel(solution.status)}</span></div><p className="mt-2 text-sm text-slate-600">{solution.description}</p><div className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><div className="rounded-lg bg-slate-50 p-3"><p className="font-semibold text-slate-500">Estimated cost</p><p className="mt-1 font-bold text-slate-900">{solution.estimatedCost || "Not provided"}</p></div><div className="rounded-lg bg-slate-50 p-3"><p className="font-semibold text-slate-500">Estimated duration</p><p className="mt-1 font-bold text-slate-900">{solution.estimatedDuration || "Not provided"}</p></div><div className="rounded-lg bg-slate-50 p-3 sm:col-span-2"><p className="font-semibold text-slate-500">Technologies</p><p className="mt-1 font-bold text-slate-900">{Array.isArray(solution.technologies) && solution.technologies.length ? solution.technologies.join(", ") : "Not provided"}</p></div></div>{solution.implementationPlan && <div className="mt-3 rounded-lg bg-slate-50 p-3 text-sm"><p className="font-semibold text-slate-500">Implementation plan</p><p className="mt-1 text-slate-700">{solution.implementationPlan}</p></div>}{solution.status === "UNIVERSITY_REVIEW" && <div className="mt-4 flex gap-2"><button disabled={busy === solution.id} onClick={() => reviewSolution(solution.id, "approve")} className="inline-flex items-center gap-2 rounded-lg bg-emerald-700 px-3 py-2 text-sm font-bold text-white"><Check size={15} /> Approve</button><button disabled={busy === solution.id} onClick={() => reviewSolution(solution.id, "request_changes")} className="rounded-lg border px-3 py-2 text-sm font-bold text-amber-700">Request changes</button><button disabled={busy === solution.id} onClick={() => reviewSolution(solution.id, "reject")} className="rounded-lg border px-3 py-2 text-sm font-bold text-rose-700">Reject</button></div>}</article>)}</div>
